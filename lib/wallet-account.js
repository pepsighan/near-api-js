...
class WalletConnection {
     ...
     */
    static createWithLocalStorage(near, appKeyPrefix) {
        ...
        wc._keyStore = near.connection.signer.keyStore;
     ...
     */
    static async createWithChromeStorage(near, appKeyPrefix) {
     ...
     */
    async _completeSignInWithAccessKey(storageType) {
        const currentUrl = new URL(window.location.href);
...
class ConnectedWalletAccount extends account_1.Account {
    ...
    async accessKeyMatchesTransaction(accessKey, receiverId, actions) {
        ...
        if (permission.FunctionCall) {
             ...
             ********************************/
            if (allowedReceiverId === this.accountId &&
